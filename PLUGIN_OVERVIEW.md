...
- **Agent Markdown that survives saving** — GitHub callouts (`> [!NOTE]`, `> [!WARNING]`, collapsible `[!TIP]-`), `<details>` blocks, footnotes, `<kbd>`, `<sub>`, `<sup>`, `==highlight==`, emoji shortcodes, raw HTML, and HTML comments — including foldable `<!-- name:start -->` … `<!-- name:end -->` regions. Opening a file never rewrites it, and an edit changes only the lines you touched.
- **Mermaid diagrams** — rendered in place. Right-click a flowchart block to rename it, add a block or a yes/no branch after it, change its shape or delete it; the change is written back into the diagram source. Templates for sequence, Gantt, class, state, ER, mind map, pie and timeline diagrams. The magnifier button opens a diagram full screen: fit to screen, zoom with buttons, ⌘+scroll or pinch, drag to pan.
